...
def load_or_initialize_json(output_json_path):
    if os.path.exists(output_json_path):
        ...
        return []
import logging
logging.basicConfig(filename='cold_log.log', level=logging.INFO, format='%(asctime)s %(levelname)s:%(message)s')
...
输出示例：该评论被认为是"有毒"，涉及的主题为‘LGBTQ’。表达方式被视为‘显性仇恨言论’，它属于‘仇恨言论’。"""
            ...
            if len(explanations) % 500 == 0:
                ...
                logging.info(f"process {len(explanations)}, save to {part_path}")
...
for i in range(4,6):  
    print(i)
    input_csv_path = f'/hpc2hdd/home/jzhao815/datagen/cold/cold_train.csv_part_{i}.csv'
    output_json_path = 'combined_resultsnew.json'
